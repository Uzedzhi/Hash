<img width="277" height="192" alt="image" src="https://github.com/user-attachments/assets/3cc0c4f8-23ec-4060-956b-c257945f9c23" /># Hash optimizing
...
|-----------------------------|----------------------|------------------|-------------|-------------------|
| AsciiSum_Hash_E.txt         | 0.347769 ± 0.003893  | 452.539650       | 7.563313    | 323404.571429     |                                 
| Rollleft_Hash_E.txt         | 0.303567 ± 0.002335  | 16.582859        | 7.563313    | 12069.142857      |                                 
| Rollright_Hash_E.txt        | 0.323492 ± 0.002031  | 47.312205        | 7.563313    | 34014.285714      |                                 
| SDBM_Hash_E.txt             | 0.307145 ± 0.003037  | 7.436230         | 7.563313    | 5537.142857       |                             
| CRC32_Hash_E.txt            | 0.311401 ± 0.001114  | 7.793101         | 7.563313    | 5792.000000       |                             
| FNV1A_Hash_E.txt            | 0.314088 ± 0.004759  | 7.476638         | 7.563313    | 5566.000000       |                             
| FirstAsciiChar_Hash_E.txt   | 2.910116 ± 0.008284  | 22041.096162     | 7.563313    | 15740718.000000   |                                    
| Strlen_Hash_E.txt           | 5.443138 ± 0.024737  | 31888.328808     | 7.563313    | 22773048.857143   |                             
| AlwaysZero_Hash_E.txt       | 84.958145 ± 0.783262 | 285904.084759    | 7.563313    | 204176586.571429  | 

Из этой таблицы Видно, что функция с наименьшей дисперсией и Хи-Квадратом - SDBM, дальнейшие замеры скорости будем проводить для нее.
Увеличим набор тестируемых слов до 10 миллионов для более точных результатов. На таком наборе 4352936/10000000 слов находятся в хэш таблице.
### На новом наборе:
  | Название функции            | Время Выполнения, с  |   Дисперсия      | Load Factor |   Хи-Квадрат      |
  |-----------------------------|----------------------|------------------|-------------|-------------------|
  | SDBM_Hash_E.txt             | 9.47769 ± 0.029786   | 7.436230         | 7.563313    | 5537.142857       | 


## Оптимизация бакетов
  До данного момента каждая ячейка хэш функции представляла из себя двусвязный список. Как известно, элементы двусвязного списка необязательно расположены строго подряд в памяти, из-за этого часто происходит промах хешей. Было решено переписать структуру ячейки и поменять список на структуру из трёх массивов: массив длин, массив хешей и массив данных. При добавлении и поиска отдельного слова в хеш функции, было решено сравнивать сначала хеши из массива хешей, потом длины слов и потом сами слова посимвольно. В отличие от предыдущего метода, где мы сразу проверяли строки посимвольно, в новом мы поняли, что большинство слов отличаются хешем и длиной слова, и легче сравнить два числа, чем сравнивать все символы подряд. 
  Это несет за собой накладные расходы на использование strlen для каждого нового слова, которое мы хотим добавить в хэш таблицу или проверить на принадлежность ей, но т.к. мы и так в прошлом методе проходили по каждому слову, то скорость упасть не должна.
  Также это помогает локальности программы - т.к. все все три массива в бакете расположены подряд (аллоцированы один calloc'ом) и, имея начальные размеры по 32 каждый, целиком помещаются в кэш. 
  | Название функции            | Время Выполнения, с  |   Дисперсия      | Load Factor |   Хи-Квадрат      |
  |-----------------------------|----------------------|------------------|-------------|-------------------|
  | SDBM_Hash_E.txt             | 9.47769 ± 0.029786   | 7.436230         | 7.563313    | 5537.142857       | 
