# Hash optimizing

Реализация структуры данных хэш-таблицы, с изначальной намеренно медленной производительностью, и впоследствии выявление слабых мест программы и улучшение их.

## Базовый Функционал хэш-таблицы
  - Каждый бакет реализован в виде двусвязного списка со вставкой и удалением за О(1)
  - Возможность применить любую хэш функцию для работы с хэш таблицей.
  - Добавление строки
  - Поиск строки

## Метод тестирования

### Обучающие наборы
Загружаемый в хэш-таблицу набор состоит из половины словаря Американского английского языка (`texts/usa.txt`). Всего в нем насчитывается 37811 уникальных слов. При количестве бакетов 4999 (простое число для минимизации коллизий) Load Factor примерно равен 7.6, что достаточно для получения заметных результатов оптимизации.

В качестве тестируемого набора была выбрана Книга "Компьютерные системы. Архитектура и программирование" Рэндала Э. Брайанта и Дэвида Р. О'Халларона, предварительно обработанная  от лишних utf-8 символов скриптом `scripts/GetRidOfDelims.py`. Количество слов: 388362

Наборы подобраны специально так, чтобы половины тестируемых слов не было в хэш таблице, а половина была. Тесты показали, что всего в тестовом наборе 235890/388362 слов находятся в хэш таблице.

### Что измеряем

Для выбранной хэш функции будем измерять общее время выполнения программы: загрузку словаря в хэш-таблицу + поиск слов из тестируемого набора.
Измерения будем проводить при помощи hyperfine.
Для повторения теста:
 ```
hyperfine ./bin/hash texts/usa.txt --warmup 5 --runs 5 --export-csv <out_file_name>.txt
 ```

Для анализа того, какая функция нуждается в оптимизации будет использовалось профилирование инструментом callgrind.
```
valgrind  --tool=callgrind
          --callgrind-out-file=callgrind.out.<out_file_name>
          ./bin/hash texts/usa.txt
```
Для удобства рассмотрения полученных данных в графическом виде используется программа `kcachegrind`.

## Первая версия: результаты
Сравнивалось 9 хеш функций:
1) ### AsciiSum
   Возвращает сумму всех ASCII кодов в строке.
   Время выполнения: 0.348 ± 0.004с
   ![](img/Ascii_Hash.png)
2) ### Rollleft
   На каждой итерации использует цикл. сдвиг влево с XOR'ом ASCII кода символа
   Время выполнения: 0.304 ± 0.002с
   ![](img/Rollleft_Hash.png)
2) ### Rollright
   Работает также как и Rollleft, только сдвиг вправо.
   Время выполнения: 0.323 ± 0.002с
   ![](img/Rollright_Hash.png)
3) ### SDBM
   Версия GNU hash, в каждой итерации происходит умножение хэша на константу 65559 и сложение с ascii кодом
   Время выполнения: 0.307 ± 0.003с
   ![](img/SDBM_Hash.png)
5) ### CRC32
   Изначально алгоритм задумывался для обнаружения случайных ошибок в сообщении, но за его быстроту он нашел себе место в множестве хэш функций.
   Время выполнения: 0.311 ± 0.001с
   ![](img/CRC32_Hash.png)
7) ### FNV1A
   Популярный алгоритм хэширования, известный за маленькое количество коллизий на средних и малых строках.
   Время выполнения: 0.314 ± 0.005с
   ![](img/FNV1A_Hash.png)
9) ### FirstAsciiChar
    Возвращает ascii код первой буквы
   Время выполнения: 2.910 ± 0.008с
   ![](img/FirstAsciiChar_Hash.png)
11) ### Strlen
    Возвращает длину строки
   Время выполнения: 5.443 ± 0.024с
   ![](img/Strlen_Hash.png)
12) ### THEBESTHASHFUNCINTHEWORLD
    Она просто возвращает 0. Все слова попадают в один бакет
   Время выполнения: 84.958 ± 0.783с
   ![](img/AlwaysZero_Hash.png)
### Таблица результатов
| Название функции            | Время Выполнения, с  |   Дисперсия      | Load Factor |   Хи-Квадрат      |
|-----------------------------|----------------------|------------------|-------------|-------------------|
| AsciiSum.txt         | 0.347769 ± 0.003893  | 452.539650       | 7.563313    | 323404.571429     |                                 
| Rollleft.txt         | 0.303567 ± 0.002335  | 16.582859        | 7.563313    | 12069.142857      |                                 
| Rollright.txt        | 0.323492 ± 0.002031  | 47.312205        | 7.563313    | 34014.285714      |                                 
| SDBM.txt             | 0.307145 ± 0.003037  | 7.436230         | 7.563313    | 5537.142857       |                             
| CRC32.txt            | 0.311401 ± 0.001114  | 7.793101         | 7.563313    | 5792.000000       |                             
| FNV1A.txt            | 0.314088 ± 0.004759  | 7.476638         | 7.563313    | 5566.000000       |                             
| FirstAsciiChar.txt   | 2.910116 ± 0.008284  | 22041.096162     | 7.563313    | 15740718.000000   |                                    
| Strlen.txt           | 5.443138 ± 0.024737  | 31888.328808     | 7.563313    | 22773048.857143   |                             
| AlwaysZero.txt       | 84.958145 ± 0.783262 | 285904.084759    | 7.563313    | 204176586.571429  | 

Из этой таблицы Видно, что функции с наименьшей дисперсией и Хи-Квадратом - SDBM, CRC32, FNV1A, дальнейшие замеры скорости будем проводить для них.
Увеличим набор тестируемых слов до 612 тысяч и будем повторять измерения 100 раз для более точных результатов. На таком наборе 32556900/61294800 слов помечаются как найденные.

### На новом наборе:
  | Название функции     | Время Выполнения, с  |
  |----------------------|----------------------|
  | SRC32.txt            | 7.060 ±  0.062       |
  | SDBM.txt             | 5.907 ±  0.101       |
  | FNV1A.txt            | 5.717 ±  0.088       |

## Профилировщик функции CRC32
  <img width="1196" height="613" alt="image" src="https://github.com/user-attachments/assets/e62841aa-65e7-4361-865a-6e547a725b60" />
  ## Профилировщик функции FNV1A
  <img width="1188" height="566" alt="image" src="https://github.com/user-attachments/assets/4fab22cc-d518-4dca-a03e-4a22167fe697" />
## Профилировщик функции SDBM
  <img width="1191" height="554" alt="image" src="https://github.com/user-attachments/assets/33e0b3c0-2780-4050-b4dc-4b90ebd628c8" />

## Оптимизация бакетов
  До данного момента каждая ячейка хэш функции представляла из себя двусвязный список. Как известно, элементы двусвязного списка необязательно расположены строго подряд в памяти, из-за этого часто происходит промах хешей. Было решено переписать структуру ячейки и поменять список на структуру из трёх массивов: массив длин, массив хешей и массив данных. При добавлении и поиска отдельного слова в хеш функции, было решено сравнивать сначала хеши из массива хешей, потом длины слов и потом сами слова посимвольно. В отличие от предыдущего метода, где мы сразу проверяли строки посимвольно, в новом мы поняли, что большинство слов отличаются хешем и длиной слова, и легче сравнить два числа, чем сравнивать все символы подряд. 
  Это помогает локальности программы - т.к. все все три массива в бакете расположены подряд (аллоцированы один calloc'ом) и, имея начальные размеры по 32 каждый, целиком помещаются в кэш. 
  
  | Название функции  | Время Выполнения, с   |
  |-------------------|-----------------------|
  | CRC32.txt         | 3.663 ±  0.057        |
  | SDBM.txt          | 2.283 ±  0.030        |
  | FNV1A.txt         | 2.335 ±  0.122        |

  Получили ускорение от 1.9 до 2.7 раз. 
  
  ## Профилировщик функции SDBM
<img width="1190" height="604" alt="image" src="https://github.com/user-attachments/assets/f00310c9-d036-4b09-b93c-8c7be83a2142" />

  ## Профилировщик функции CRC32
<img width="1183" height="627" alt="CRC32" src="https://github.com/user-attachments/assets/92cfa89f-777c-4264-9179-4788538f3d33" />

  ## Профилировщик функции FNV1A
  <img width="1204" height="637" alt="FNV1A" src="https://github.com/user-attachments/assets/20bdc315-1284-4f57-b4e2-47ff9f5fae3b" />
    
## Оптимизация Хеш функций
  Оптимизируем хеш функции SDBM, FNV1A и CRC32, переписав их на ассемблере
  | Название функции     | Время Выполнения, с  |
  |----------------------|----------------------|                           
  | MySDBM.txt           | 2.249 ± 0.011        |                           
  | MyCRC32.txt          | 2.291 ± 0.025        |                           
  | MyFNV1A.txt          | 2.309 ± 0.026        |

  Получили ускорение в 1.6 раз для функции MyCRC32
OPT: 4.705 s ±  0.436
  <img width="1198" height="559" alt="image" src="https://github.com/user-attachments/assets/c05f89ac-1313-47d4-9aec-de104a209ef0" />

## Оптимизация Strlen и Strcmp
  Видим, что в среднем функции strlen + strcmp занимают около 18% всего времени. Попробуем их оптимизировать с помощью:
  - __asm вставки
  - отдельной asm функции

| Название функции            | Тип программы  | Время Выполнения, с  |
|-----------------------------|----------------------|------------------|
| SDBM.txt                    | ASM                | 2.108 ± 0.034  |                       
| CRC32.txt                   | ASM                | 2.078 ± 0.021  |                         
| FNV1A.txt                   | ASM                | 2.172 ± 0.027  |
| SDBM.txt                    | ASMINLINE  | 2.084 ± 0.045  |                        
| CRC32.txt                   | ASMINLINE  | 2.012 ± 0.031  |                  
| FNV1A.txt                   | ASMINLINE  | 2.071 ± 0.023  |

Получили ускорение примерно в 1.1 раз.

## Профилировщик CRC32 после strcmp, strlen
<img width="1186" height="624" alt="image" src="https://github.com/user-attachments/assets/75a29060-05b5-4597-8fbd-bee0fe101404" />


## Итоги
Обще ускорение программы составило 3.52 раз
